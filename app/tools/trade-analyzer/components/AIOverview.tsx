...
export function AIOverview({ sideA, sideB, isDynasty }: Props) {
  ...
  async function generate() {
    ...
    try {
      const res = await fetch("/api/trade-analyzer/overview", {
        ...
        body: JSON.stringify({
          sideA: sideA.map((p) => ({ name: p.name, position: p.position, value: p.value, age: p.age })),
          sideB: sideB.map((p) => ({ name: p.name, position: p.position, value: p.value, age: p.age })),
          isDynasty,
        }),
      });
    ...
    <div className="rounded-2xl border border-zinc-200 dark:border-zinc-800/60 bg-white dark:bg-zinc-900/20 px-5 py-4">
      <div className="flex items-center justify-between gap-4">
        <div className="flex items-center gap-2">
          ...
          <span className="text-[10px] text-zinc-500 dark:text-zinc-600">powered by Claude</span>
        </div>
        <button
          onClick={generate}
          disabled={loading}
