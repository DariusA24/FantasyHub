...
export function FreeTradeSide({ label, players, rawTotal, benchBonus, isDynasty, valueMap, excluded, onAdd, onRemove, accent }: {
  label: string;
  ...
  const adjustedTotal = rawTotal + benchBonus;
        ...
        <div className="flex items-center gap-1.5">
          {benchBonus > 0 && (
            <span className="text-[10px] text-zinc-500">
              {rawTotal.toLocaleString()} <span className="text-emerald-500">+{benchBonus}</span>
            </span>
          )}
