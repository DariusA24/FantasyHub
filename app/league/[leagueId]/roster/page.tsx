...
function scoringLabel(ls: LeagueSettings): string {
  const type = ls.settings?.type;
  if (type === 2) return "Dynasty";
  const positions = ls.roster_positions ?? [];
  if (positions.includes("SUPER_FLEX")) return "2QB / SuperFlex";
  const rec = ls.scoring_settings?.rec ?? 0;
  if (rec >= 1) return "PPR";
...
export default function PublicRosterPage() {
  ...
  useEffect(() => {
    ...
    if (!leagueId) { setError("Invalid league id."); setLoading(false); return; }
    ...
    const load = async () => {
      ...
      try {
        const [rostersRes, leagueRes, usersRes] = await Promise.all([
          fetch(`/api/sleeper/rosters/${encodeURIComponent(leagueId)}`),
          fetch(`/api/sleeper/league/${encodeURIComponent(leagueId)}`),
          fetch(`/api/sleeper/league/${encodeURIComponent(leagueId)}/users`),
        ]);

        if (!rostersRes.ok) { setError("Failed to load rosters."); setLoading(false); return; }

        if (leagueRes.ok) setLeagueSettings(await leagueRes.json());
        if (usersRes.ok) setLeagueUsers(await usersRes.json());

        const rosters: SleeperRoster[] = await rostersRes.json();
        if (!Array.isArray(rosters) || rosters.length === 0) {
          setError("No rosters found."); setLoading(false); return;
        }
        setAllRosters(rosters);

        // Try to identify "my" roster if signed in
        let mySleeperUserId: string | null = null;
        if (isSignedIn) {
          const profileRes = await fetch("/api/profile");
          if (profileRes.ok) {
            const pd = await profileRes.json();
            mySleeperUserId = pd?.profile?.sleeperProfileId ?? null;
          }
        }

        const userRoster = (mySleeperUserId ? rosters.find(r => r.owner_id === mySleeperUserId) : null) ?? rosters[0];
        setMyRoster(userRoster ?? null);
        setViewingRosterId(userRoster?.roster_id ?? null);
  ...
  useEffect(() => {
    ...
    fetch(`/api/dynasty-rankings/all?numQbs=${isSF ? 2 : 1}`)
      .then(r => r.ok ? r.json() : Promise.reject())
      .then((data: any[]) => {
        const map: Record<string, any> = {};
        for (const e of data) if (e.player?.sleeperId) map[e.player.sleeperId] = e;
        setDynastyMap(map);
      })
      .catch(() => {});
  }, [leagueSettings]);
  ...
  useEffect(() => {
    ...
    fetch("/api/start-sit/nfl-state")
      .then(r => r.json())
      .then(async (state: { week: number; display_week: number; season: string }) => {
        const week = Math.max(state.display_week ?? 1, 1);
        const season = state.season;
        const [projRes, statsRes] = await Promise.all([
          fetch(`/api/start-sit/projections?week=${week}&season=${season}`),
          week > 1 ? fetch(`/api/start-sit/stats?week=${week - 1}&season=${season}`) : Promise.resolve(null),
        ]);
        if (projRes?.ok) {
  ...
  const { starters, bench } = useMemo(() => {
    ...
    return {
      starters: displayedRoster.starters ?? [],
      bench: displayedRoster.players.filter(id => !startersSet.has(id)),
    };
  }, [displayedRoster]);
  ...
  if (loading) return pageShell(
    <div className="mt-6 rounded-2xl border border-[#1d212b] bg-[#050609]/80 p-6 animate-pulse">
  ...
  if (error) return pageShell(
    <div className="mt-4 rounded-2xl border border-red-900/60 bg-red-950/30 p-5">
      ...
      <button className="text-sm text-[#F4D06F] hover:underline" onClick={() => window.history.back()}>← Go back</button>
    </div>
  );

  const selectedPlayer = selectedPlayerId ? player[selectedPlayerId] ?? null : null;
  ...
  const isViewingFirst = displayedRoster?.roster_id === myRoster?.roster_id;
    ...
    <div className="hub-page">
      <div className="mx-auto max-w-6xl px-4 pb-24 pt-6 text-gray-800 dark:text-zinc-200">
        ...
        <div className="mb-6 flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
          <div>
            <div className="inline-flex items-center gap-2 rounded-full bg-zinc-100 dark:bg-zinc-900/70 border border-zinc-300 dark:border-zinc-700/70 px-3 py-1 mb-2">
              ...
              <span className="text-xs font-medium uppercase tracking-wide text-zinc-600 dark:text-zinc-300">Active League</span>
            </div>
          </div>
        </div>
          ...
          <div className="mb-5">
            ...
            <div className="flex gap-2 overflow-x-auto pb-1 scrollbar-hide">
              {allRosters.map(roster => {
                ...
                const av = owner?.avatar ? `https://sleepercdn.com/avatars/thumbs/${owner.avatar}` : null;
          ...
          <section className="hub-card shadow-[0_0_25px_rgba(0,0,0,0.6)] p-4 md:p-5 mb-10">
            <div className="mb-4 flex items-center justify-between">
              <div>
                ...
                <p className="text-xs text-gray-500 dark:text-zinc-400">Active lineup for this week.</p>
              </div>
            ...
            {starters.length > 0 ? (
              <>
                ...
                <ul className="divide-y divide-zinc-800/40 text-sm">
                  {starters
                    .filter(id => posFilter === "ALL" || player[id]?.position === posFilter)
                    .map(playerId => {
                      ...
                      return (
                        ...
                        >
                          <img src={playerThumb(playerId)} alt={p?.full_name ?? playerId}
                            className="h-9 w-9 rounded-full object-cover bg-zinc-200 dark:bg-zinc-800 border border-zinc-300 dark:border-zinc-700/60"
                            onError={e => { (e.target as HTMLImageElement).src = "/default-profile.png"; }} />
                          <div className="min-w-0">
                            <p className="font-medium text-gray-900 dark:text-zinc-100 truncate">{p?.full_name ?? playerId}</p>
                          ...
                          <p className="text-xs font-semibold text-zinc-700 dark:text-zinc-300 text-right">
                            {projMap[playerId] != null ? projMap[playerId].toFixed(1) : <span className="text-zinc-500">—</span>}
                          </p>
                          <p className="text-xs font-semibold text-amber-600 dark:text-[#F4D06F] text-right">
                            {statsMap[playerId] != null ? statsMap[playerId].toFixed(1) : <span className="text-zinc-500">—</span>}
                          </p>
          ...
          <section className="hub-card shadow-[0_0_18px_rgba(0,0,0,0.5)] p-4 md:p-5 mb-10">
            <div className="mb-4 flex items-center justify-between">
              <div>
                ...
                <p className="text-xs text-gray-500 dark:text-zinc-400">Depth pieces and future starters.</p>
              </div>
        ...
        <PlayerStatsModal
          ...
          isSuperFlex={(leagueSettings?.roster_positions as string[] | undefined)?.includes("SUPER_FLEX") ?? false}
          onClose={() => { setIsStatsOpen(false); setSelectedPlayerId(null); }}
        />
      </div>
    </div>
  );
